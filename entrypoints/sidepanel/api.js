...
export async function getGeminiResources({ title, description, apiKey }) {
    // console.log(apiKey);
  try {
    ...
    const data = await res.json();
    return data.hint || "No resources generated";
  } catch (error) {
    // console.error("Error fetching Gemini hint:", error);
    return "Failed to generate resources. Please try again.";
  }
}
